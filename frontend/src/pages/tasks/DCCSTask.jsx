...
function DCCSTask({ config, onComplete }) {
  ...
  const shapes = config.shapes || ['star', 'circle', 'diamond'];
  const colors = config.colors || ['#e74c3c', '#3498db', '#2ecc71'];
  ...
  useEffect(() => {
    ...
    if (mode === 'blocked') {
      ...
      for (let i = 0; i < totalTrials; i++) {
        ...
        // Ensure card matches exactly one target by chosen rule
        const correctTarget = Math.random() < 0.5 ? 0 : 1;
        const card = { shape, color };
        if (rule === 'shape') {
          card.shape = targets[correctTarget].shape;
          // Make color different from the matched target
          card.color = colors.find(c => c !== targets[correctTarget].color) || colors[0];
        } else {
          card.color = targets[correctTarget].color;
          card.shape = shapes.find(s => s !== targets[correctTarget].shape) || shapes[0];
        }
